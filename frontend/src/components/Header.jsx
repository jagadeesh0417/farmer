...
const QUICK_LINKS = [
  { label: 'Deal of the Week', to: '/products' },
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-3 sm:px-8 lg:px-10">
          ...
          <nav className="hidden lg:flex items-center gap-1">
            <div className="relative" onMouseEnter={() => setMegaOpen(true)} onMouseLeave={() => setMegaOpen(false)}>
              <button className={`px-3 py-2 text-[12px] font-medium text-ink hover:text-green-600 transition-colors ${megaOpen ? 'text-green-600' : ''}`}>
                Shop <span className="text-[9px] ml-0.5">▼</span>
              </button>
              {megaOpen && (
                <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-[640px] bg-white border border-border rounded-xl shadow-lg py-6 px-8 z-50" onMouseEnter={() => setMegaOpen(true)} onMouseLeave={() => setMegaOpen(false)}>
            ...
            {QUICK_LINKS.map(item => (
              <Link key={item.label} to={item.to} onClick={closeAll} className="px-3 py-2 text-[12px] font-medium text-ink hover:text-green-600 transition-colors">{item.label}</Link>
            ))}
          </nav>
      ...
      {menuOpen && (
        <div className="lg:hidden fixed inset-0 z-50 bg-white flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-1">
            ...
            <div>
              <button onClick={() => setMegaOpen(!megaOpen)} className="flex w-full items-center justify-between rounded-lg px-4 py-3 text-sm font-semibold text-ink hover:bg-green-50">
                Shop <span className={`text-[10px] transition-transform ${megaOpen ? 'rotate-180' : ''}`}>▼</span>
              </button>
              {megaOpen && (
            ...
            {[{ label: 'Deal of the Week', to: '/products' }, { label: 'Blog', to: '/#journal' }, { label: 'Store Locator', to: '/#contact' }].map(item => (
              <NavLink key={item.label} to={item.to} onClick={closeAll} className={({ isActive }) => `block rounded-lg px-4 py-3 text-sm font-semibold ${isActive ? 'bg-green-50 text-green-600' : 'text-ink hover:bg-green-50 hover:text-green-600'}`}>{item.label}</NavLink>
            ))}
