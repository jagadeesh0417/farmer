...
export default function Header() {
  ...
  return (
    <>
      {/* Announcement bar */}
      <div className="bg-green-700 text-white text-nav font-medium tracking-[0.05em] overflow-hidden h-9 flex items-center">
      ...
      <header className={`sticky top-0 left-0 right-0 z-50 transition-all duration-300 bg-white ${scrolled ? 'shadow-sm' : 'shadow-none'}`}>
        <div className="section-container flex items-center justify-between py-3">
          ...
          <Link to="/" className="flex items-center gap-2 shrink-0" onClick={closeAll}>
            {logo ? (
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="font-heading text-h3 font-bold text-green-700 tracking-tight">HaiFarmer</span>
              </div>
            )}
          </Link>
          ...
          <nav className="hidden lg:flex items-center gap-1 font-menu">
            <NavLink to="/" end className={({ isActive }) => `px-3 py-2 text-nav font-medium transition-colors ${isActive ? 'text-green-600' : 'text-ink hover:text-green-600'}`}>Home</NavLink>
            <NavLink to="/products" className={({ isActive }) => `px-3 py-2 text-nav font-medium transition-colors ${isActive ? 'text-green-600' : 'text-ink hover:text-green-600'}`}>Products</NavLink>
            <NavLink to="/combos" className={({ isActive }) => `px-3 py-2 text-nav font-medium transition-colors ${isActive ? 'text-green-600' : 'text-ink hover:text-green-600'}`}>Combos</NavLink>
            <NavLink to="/about" className={({ isActive }) => `px-3 py-2 text-nav font-medium transition-colors ${isActive ? 'text-green-600' : 'text-ink hover:text-green-600'}`}>Our Story</NavLink>
            <Link to="/impact" className="px-3 py-2 text-nav font-medium text-ink hover:text-green-600 transition-colors">Impact</Link>
            <Link to="/journal" className="px-3 py-2 text-nav font-medium text-ink hover:text-green-600 transition-colors">Journal</Link>
            <Link to="/contact" className="px-3 py-2 text-nav font-medium text-ink hover:text-green-600 transition-colors">Contact</Link>
          </nav>
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            <a href="https://wa.me/919709704563?text=Hello%20HaiFarmer" target="_blank" rel="noopener noreferrer"
              className="hidden lg:inline-flex items-center gap-1.5 bg-green-600 text-white px-4 py-2 rounded-lg text-nav font-semibold hover:bg-green-700 transition-colors">
            ...
            <div>
              <div className="flex items-center justify-between rounded-lg px-4 py-3">
                ...
                <button onClick={() => setMegaOpen(megaOpen === 'story-mobile' ? false : 'story-mobile')} className="p-1 text-muted hover:text-ink">
                  <span className={`text-micro transition-transform ${megaOpen === 'story-mobile' ? 'rotate-180' : ''}`}>▼</span>
                </button>
              </div>
