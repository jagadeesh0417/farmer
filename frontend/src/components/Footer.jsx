    ...
    <footer className="bg-[#1a2e1a] text-white/90 font-menu">
      ...
      <div className="border-b border-white/10">
        <div className="section-container py-10 md:py-12">
          <div className="max-w-xl mx-auto text-center">
            <h3 className="font-heading text-h3 font-bold text-white">Join the HaiFarmer Family</h3>
            <p className="mt-2 text-body-sm text-white/60">Subscribe for exclusive offers, wellness tips, and new product updates.</p>
            <form className="mt-5 flex gap-2 max-w-md mx-auto" onSubmit={e => e.preventDefault()}>
              <input type="email" placeholder="Your email address" required
                className="flex-1 px-4 py-2.5 rounded-lg bg-white/10 border border-white/20 text-white placeholder:text-white/40 text-body-sm outline-none focus:border-green-400 transition-colors" />
              <button type="submit"
                className="px-5 py-2.5 rounded-lg bg-green-600 text-white text-body-sm font-semibold hover:bg-green-500 transition-colors shrink-0">Subscribe</button>
            </form>
          </div>
        </div>
      </div>
      ...
      <div className="section-container py-10 md:py-14">
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-8">
          ...
          {/* Account */}
          <div>
            <h4 className="text-nav font-semibold tracking-[0.1em] uppercase text-white/80 mb-4">Account</h4>
            <ul className="space-y-2.5">
              {[
          ...
          <div>
            ...
            <div className="space-y-3 text-body-sm text-white/50">
              ...
              <div className="flex items-start gap-2">
                <svg className="h-4 w-4 mt-0.5 shrink-0 text-green-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
        ...
        <div className="section-container py-6 flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            {[
              ...
              <a key={social.label} href={social.href} target="_blank" rel="noopener noreferrer" aria-label={social.label}
                ...
                <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24">
                  ...
                  {social.label === 'Facebook' && <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>}
                  {social.label === 'YouTube' && <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/>}
                  {social.label === 'X (Twitter)' && <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>}
                </svg>
              </a>
            ))}
          </div>
          <div className="flex items-center gap-2">
            {['Visa', 'Mastercard', 'UPI', 'COD'].map(method => (
              <span key={method} className="text-micro font-semibold text-white/30 uppercase border border-white/20 rounded px-2 py-0.5">{method}</span>
            ))}
          </div>
        </div>
      </div>
