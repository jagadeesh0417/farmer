  ...
  if (!product) return (
    <div className="flex min-h-[60vh] items-center justify-center bg-white">
      <div className="text-center">
        <p className="text-body-sm font-semibold text-ink">Product not found</p>
        <Link to="/products" className="mt-4 inline-flex items-center gap-2 bg-green-600 text-white px-6 py-2.5 rounded-lg text-body-sm font-semibold hover:bg-green-700 transition-colors">Back to Products</Link>
      </div>
    </div>
  )
      ...
      <div className="section-container py-8 lg:py-12">
        <div className="grid gap-8 lg:grid-cols-2 lg:gap-14">
          ...
          <div className="flex flex-col gap-5">
            {categoryTag && <span className="text-micro font-semibold tracking-[0.1em] uppercase text-green-600">{categoryTag}</span>}
            ...
            <div className="flex flex-wrap gap-4">
              {KEY_BENEFITS.map(b => (
                <div key={b.label} className="flex items-center gap-1.5 text-caption text-muted"><span>{b.icon}</span><span>{b.label}</span></div>
              ))}
            </div>

            {product.product_variants?.length > 0 && (
              <div>
                <label htmlFor="variant-select" className="text-micro font-semibold tracking-[0.1em] uppercase text-muted mb-2 block">Select Size Pack</label>
                <div className="relative">
                  <select
                    ...
                    className="appearance-none h-[56px] w-full rounded-full border-2 border-[#222] bg-white px-5 pr-12 font-product text-body font-semibold text-[#111] outline-none focus:border-[#0E9F3E]"
                  >
            ...
            <div className="flex items-baseline gap-3">
              ...
              {mrp > price && <span className="font-product text-price text-muted-light line-through">{formatPrice(mrp)}</span>}
              {savings > 0 && <span className="font-product text-caption font-semibold text-sale bg-sale-light px-2 py-0.5 rounded">Save {formatPrice(savings)}</span>}
            </div>
            ...
            <div className="flex gap-3 items-center">
              <div className="flex items-center rounded-lg border border-border bg-white">
                ...
                <span className="min-w-[2.5rem] text-center text-body-sm font-semibold text-ink font-product">{quantity}</span>
              ...
              {inCartQty > 0 && <span className="text-caption text-muted font-product">({inCartQty} in cart)</span>}
            </div>

            <div className="flex gap-3">
              <button onClick={handleAddToCart} className="flex-1 bg-green-600 text-white px-6 py-3 rounded-lg text-btn font-semibold hover:bg-green-700 transition-colors active:scale-[0.98] font-product">Add to Cart</button>
              <button className="flex-1 bg-ink text-white px-6 py-3 rounded-lg text-btn font-semibold hover:bg-ink/80 transition-colors active:scale-[0.98] font-product">Buy it Now</button>
            </div>
          </div>
        </div>
          ...
          {[
            ...
            <div key={tab.key} className="border-b border-border last:border-b-0">
              <button onClick={() => setAccordion(accordion === tab.key ? null : tab.key)} className="flex w-full items-center justify-between py-4 px-1 text-left">
                <span className="text-body-sm font-semibold text-ink">{tab.label}</span>
              ...
              {accordion === tab.key && (
                <div className="pb-4 px-1 text-body-sm text-muted leading-relaxed">{tab.content}</div>
              )}
            </div>
          ))}
        </div>
        ...
        {/* Certifications */}
        <div className="mt-10">
          <h3 className="text-body-sm font-semibold text-ink mb-4">Our Certifications</h3>
          <div className="flex flex-wrap gap-3">
            {CERTIFICATIONS.map(c => (
              <div key={c.name} className="flex items-center gap-2 border border-border rounded-lg px-4 py-2">
                ...
                <span className="text-caption font-medium text-muted">{c.name}</span>
              </div>
            ))}
            <button className="text-caption font-semibold text-green-600 hover:text-green-700 border border-border rounded-lg px-4 py-2">View All</button>
          </div>
        </div>

        <div className="mt-8 flex flex-wrap gap-4">
          {VALUES_ROW.map(v => (
            <span key={v.label} className="text-micro font-medium text-muted uppercase tracking-[0.05em] border border-border rounded px-2.5 py-1">{v.label}</span>
          ))}
        </div>
        ...
        <div className="mt-12 bg-sand rounded-xl overflow-hidden">
          <div className="grid lg:grid-cols-2">
            <div className="p-8 lg:p-10 flex flex-col justify-center">
              ...
              <Link to="/farmers" className="mt-4 text-body-sm font-semibold text-green-600 hover:text-green-700 transition-colors">Meet the Farmers →</Link>
            </div>
